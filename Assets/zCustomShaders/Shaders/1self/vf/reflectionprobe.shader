	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float3 r=reflect(-viewDir,i.wNormal);
				//通过BoxProjectedCubemapDirection函数修正reflectDir
				r = BoxProjectedCubemapDirection( r, i.wPos, unity_SpecCube0_ProbePosition, unity_SpecCube0_BoxMin, unity_SpecCube0_BoxMax);

				float4 envir=UNITY_SAMPLE_TEXCUBE(unity_SpecCube0,r);
				...
				return float4(col.rgb*(envirColor)+speColor,1);
			}
			ENDCG
		}
	}
